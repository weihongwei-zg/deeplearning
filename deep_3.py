...
def ward(L,n,m,t):#对参数进行初始化
    np.random.seed(1)
    w = []
    b = []
    for i in range(0,L):
        if i != 0 and i != L-1:
            p = np.random.randn(t, t)*0.01
        elif i == 0:
            p = np.random.randn(t,m)*0.01
        else:
            p = np.random.randn(1,t)*0.01
        w.append(p)
        b.append(1)
    '''
...
def forward(w,b,a,Y,L,m):#前向传播
    ...
    J = np.mean(-1 * Y * np.log(a) + (1 - Y) * np.log(1 - a))#损失函数
    return z,a,J
def backward(w,b,X,Y,learning,m,L):#反向传播
    ...
    for i in range(L-1,0,-1):
        if i == L-1:
            ...
            dz = np.dot(w[i+1].T,dz)*g1(z[i])
        dw = 1/m * np.dot(dz,tanh(z[i-1]).T)
        db = 1/m * np.sum(dz,axis=1,keepdims=True)
        w[i] -= learning*dw
        b[i] -= learning*db

        #b[i] = np.mean(b[i] - learning*db)
    dz = np.dot(w[1].T,dz)*g1(z[0])
...
if __name__ == "__main__":
    ...
    print("测试集_标签的维数 : " + str(test_set_y.shape))
    print()
    w,b = ward(L,train_set_x.shape[1],train_set_x.shape[0],t)
    for i in range(5000):
