...
def g1(z):  # 用tanh函数作为激活函数
    e1 = np.exp(z)
    e2 = np.exp(-z)
    return (e1 - e2) / (e1 + e2)

def g2(z):  # sigmoid函数作为激活函数
    return 1 / (1 + np.exp(-z))
...
def g_2(a):
    return a*(1-a)
def rand(n, m, t):
    w1 = np.random.randn(t, m)*0.01
    w2 = np.random.randn(1, t)*0.01  # 随机从产生符合正态分布的数
    b1 = np.zeros((t, 1), dtype='float')  # 初始化为0
...
if __name__ == "__main__":
    ...
    n, m, t, i = train_set_x.shape[1], train_set_x.shape[0], 4, 0#n表示样本个数，m表示特征个数，t表示节点个数
